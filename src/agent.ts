...
function wantsIdea(text: string): boolean {
  return /idea|hypothesis|strategy|전략|아이디어|가설/i.test(text);
}
...
function safeProviderPrompt(run: Omit<AgentRun, 'provider_response' | 'report'>): string {
  return [
    ...
    'Return Korean-friendly beginner guidance with: local evidence, missing evidence, next TossQuant commands, and verification cautions.',
    '',
    'Session handoff:',
...
export async function runAgent(request: string, options: AgentOptions = {}): Promise<AgentRun> {
  ...
  recordSessionEvent(session, { at: options.now, type: 'user.request', summary: requestPreview, payload: { provider, allow_downloads: Boolean(options.allowDownloads) } });
  ...
  const partial: Omit<AgentRun, 'provider_response' | 'report'> = { ok: true, request: requestPreview, session, provider, symbols, steps, skipped };
